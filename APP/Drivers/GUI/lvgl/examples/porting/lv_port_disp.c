...
static void disp_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p)
{
    ...
    int32_t x;
    int32_t y;
    //LCD_SetWindows(area->y1,area->x1,area->y2,area->x2);
    for (y = area->y1; y <= area->y2; y++)
    {
        for (x = area->x1; x <= area->x2; x++)
        {
            ...
            /* put_px(x, y, *color_p)*/
            LCD_DrawPoint_lvgl(x, y, color_p->full);
            color_p++;
        }
    }
    ...
    // LCD_RS_SET;
    // LCD_CS_CLR;
    // for(i=0;(i<2*(area->y2-area->y1)*(area->x2-area->x1))%100;i++);
    // {
    //   //  HAL_SPI_Transmit(&hspi1, temp+i*100, 100, 1000);
    // }
    // LCD_CS_SET;
    // for(y = area->y1; y <= area->y2; y++) {
    //     for(x = area->x1; x <= area->x2; x++) {
    //         /* Put a pixel to the display. For example: */
    //         /* put_px(x, y, *color_p)*/
    //         gram[2 * (y * lcddev.width + x)] = (color_p->full) >> 8;
    //         gram[2 * (y * lcddev.width + x) + 1] = (color_p->full);
    //         //LCD_DrawPoint_lvgl(x,y,color_p->full);
    //         color_p++;
    //     }
    // }
